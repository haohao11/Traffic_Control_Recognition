...
def data_partition(data, args, data_summary=False):
    ...
    split_n = int(len(junc_arms)*split)
    np.random.seed(6) 
    index = np.random.choice(junc_arms.shape[0], split_n, replace=False) 
    train_arms = junc_arms[index]
    # data = np.reshape(data, (-1, window_size, data.shape[-1]))
    train_val_split = []
    
    for i in data[:, 0, 5]:
        train_val_split.append(i in train_arms)
    # print(train_val_split)    
    # print("junc_arms", junc_arms)
